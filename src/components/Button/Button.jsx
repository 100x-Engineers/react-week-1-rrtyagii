...
export default function Button ({children, variant = 'solid', type = 'primary', isDisabled = false, ...rest}) {
  const styles = {
    common: `inline-flex justify-center items-center rounded-3xl shadow-lc ${rest.padding ? rest.padding : "px-6 py-2"} m-2 font-bold text-base leading-normal font-inter ${rest.width ? rest.width : "w-80"} ${rest.gap ? rest.gap : ""} ${rest.position ? rest.position : ""} ${rest}`,
